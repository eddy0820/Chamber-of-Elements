...
{
    public static void DealDamageToEverything(AffinityTypes damageType, AffinityTypes secondaryAffinityType, float value)
    {
        Player.Instance.Stats.TakeDamage(value, damageType, secondaryAffinityType, null);
        GameManager.Instance.Enemy.Stats.TakeDamage(value, damageType, secondaryAffinityType, Player.Instance);

        if(Player.Instance.MinionExists)
        {
            Player.Instance.Minion.Stats.TakeDamage(value, damageType, secondaryAffinityType, null);
        }
    ...
    public static void DealDamageToEverythingExceptEnemy(AffinityTypes damageType, AffinityTypes secondaryAffinityType, float value)
    {
        Player.Instance.Stats.TakeDamage(value, damageType, secondaryAffinityType, null);
